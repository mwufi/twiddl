...
response = client.get_user(username=screen_name, user_fields=[
                           'created_at', 'description', 'profile_image_url', 'public_metrics', 'url'])
user = response.data
...
for response in tweepy.Paginator(
        client.get_users_following, user.id, user_fields=[
            'created_at', 'description', 'profile_image_url', 'public_metrics', 'url'],
        max_results=1000, limit=2):
...
for user in query:
    print('%s has %d followers! Visit them at %s' %
